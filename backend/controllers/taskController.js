...
export const createTask = async (req, res) => {
    try{
        ...
        const activity = {
            type: "assigned",
            activity: `Task has been assigned to ${team.length} members`,
            by: userId, 
        };
...
export const dashboardStatistics = async (req, res) => {
    try{
        ...
        const users = (await User.find({isActive : true }).select("name title role isAdmin createdAt").limit(10))
        .sort({ _id: -1 });
        ...
        //group task by proirity

        const groupDate = Object.entries(
...
export const trashTask = async (req, res) => {
    try{
        const { id } = req.params
        const taskToTrash = await Task.findById(id);
        taskToTrash.isTrashed = true;
        await taskToTrash.save();
